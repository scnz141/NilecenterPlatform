-- Nile Learn Phase 6H3 semantic assertions. Always rolled back.
...
begin
  select * into strict context from public.resolve_moodle_grade_outcome_context(
  ...
  if context.active_role <> 'student' or context.projection_audience <> 'learner'
    ...
    or context.user_mapping_status <> 'exact' or context.grade_item_mapping_status <> 'exact' then
  ...
  begin
    perform * from public.resolve_moodle_grade_outcome_context(
      '40000000-0000-4000-8000-000000000003',
      '50000000-0000-4000-8000-000000000003',
      'b6000000-0000-4000-8000-000000000001',
      'd3000000-0000-4000-8000-000000000001');
    raise exception 'Registrar received grade outcome context';
  ...
  begin
    perform * from public.resolve_moodle_grade_outcome_context(
      ...
      'b6000000-0000-4000-8000-000000000001',
      'd3000000-0000-4000-8000-000000000001');
    raise exception 'Branch Admin received grade outcome context';
  exception when insufficient_privilege then null;
  end;

  select * into strict freshness from public.list_authorized_moodle_grade_outcome_freshness(
    '40000000-0000-4000-8000-000000000001',
    '50000000-0000-4000-8000-000000000001',
    'ba000000-0000-4000-8000-000000000001',
    'b6000000-0000-4000-8000-000000000001',
    'd3000000-0000-4000-8000-000000000001', '2026-07-17T12:10:00Z'
  );
  ...
  select * into strict freshness from public.list_authorized_moodle_grade_outcome_freshness(
    ...
    'ba000000-0000-4000-8000-000000000001',
    'b6000000-0000-4000-8000-000000000001',
    'd3000000-0000-4000-8000-000000000001', '2026-07-17T14:30:00Z'
  );
  ...
  select * into strict replay from public.record_moodle_grade_outcome_observation(
  ...
  if replay.replayed is not true then
    raise exception 'Idempotent grade outcome replay created a duplicate';
  end if;

  begin
    perform * from public.record_moodle_grade_outcome_observation(
      'phase6h3.fake.grade.person',
      'ba000000-0000-4000-8000-000000000001',
      'b3000000-0000-4000-8000-000000000001',
      'b6000000-0000-4000-8000-000000000001',
    ...
    raise exception 'Conflicting grade outcome replay was accepted';
  exception when unique_violation then null;
  end;

  if nile_private.moodle_grade_outcome_payload_is_safe(
    pg_catalog.jsonb_set(person_payload, '{learners,0}',
      person_payload->'learners'->0 || '{"gradingState":"graded"}'::jsonb), 'learner') then
    raise exception 'Learner received unreleased grade or feedback';
  end if;
  if nile_private.moodle_grade_outcome_payload_is_safe(
    pg_catalog.jsonb_set(person_payload, '{learners,0,feedback}', '"<unsafe>"'::jsonb),
    'person_level') then
    raise exception 'Unsafe released feedback was accepted';
  end if;
  if nile_private.moodle_grade_outcome_payload_is_safe(
    '{"internalCourseId":"b3000000-0000-4000-8000-000000000001","internalClassGroupId":"b6000000-0000-4000-8000-000000000001","gradeItemProjectionId":"d3000000-0000-4000-8000-000000000001","providerState":"available","mappingStatus":"exact","learnerCount":1,"gradedCount":1,"releasedCount":1,"feedbackReleasedCount":1,"internalUserId":"40000000-0000-4000-8000-000000000001"}'::jsonb,
    'aggregate') then
  ...
  select * into strict freshness from public.list_authorized_moodle_grade_outcome_freshness(
    ...
    'b6000000-0000-4000-8000-000000000001',
    'd3000000-0000-4000-8000-000000000001', '2026-07-17T12:10:00Z');
  if freshness.grade_item_mapping_status <> 'missing'
    or freshness.freshness_state <> 'reconciliation' or freshness.sanitized_payload is not null then
    raise exception 'Missing grade item mapping retained outcome payload: %', freshness;
  end if;
  update public.external_records set sync_state = 'matched'
  where id = 'd3000000-0000-4000-8000-000000000001';

  begin
    update public.moodle_grade_outcome_observations set outcome = outcome
    where idempotency_key = 'phase6h3.fake.grade.person';
    raise exception 'Immutable grade outcome accepted an update';
  ...
  select * into strict purged from public.purge_moodle_grade_outcome_observations(
    '2026-07-17T14:30:00Z', 1);
  if purged.deleted_count <> 1 then
    raise exception 'Bounded grade outcome purge mismatch: %', purged;
  end if;
end;
$$;
