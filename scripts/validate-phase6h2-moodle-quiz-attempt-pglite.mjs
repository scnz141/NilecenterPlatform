...
const sql = {
  ...
  seed: read("supabase/manual/112_phase6h2_moodle_quiz_attempt_fake_seed.sql"),
  assertions: read(
    "supabase/manual/212_phase6h2_moodle_quiz_attempt_assertions.sql"
  ),
  rollback: read(
    "supabase/manual/912_phase6h2_moodle_quiz_attempt_rollback.sql"
  ),
};
...
async function assertRuntimeDenials(database) {
  ...
  for (const role of ["anon", "authenticated", "service_role"]) {
    await expectDenied(
      database,
      role,
      "select * from public.moodle_quiz_attempt_observations",
      "Direct Phase 6H2 table read"
    );
    count += 1;
  }
  const calls = [
    `select * from public.resolve_moodle_quiz_attempt_context(
      '40000000-0000-4000-8000-000000000001'::uuid,
      '50000000-0000-4000-8000-000000000001'::uuid,
      'b6000000-0000-4000-8000-000000000001'::uuid,
      'd1000000-0000-4000-8000-000000000002'::uuid)`,
    `select * from public.list_authorized_moodle_quiz_attempt_freshness(
      '40000000-0000-4000-8000-000000000001'::uuid,
      '50000000-0000-4000-8000-000000000001'::uuid,
    ...
    `select * from public.purge_moodle_quiz_attempt_observations(
  ...
  try {
    ...
    for (const [user, grant, role, audience, authorizedCount] of contexts) {
      const result = await database.query(`
        select active_role, projection_audience, authorized_user_ids,
               ...
               user_mapping_status, quiz_mapping_status
    ...
    for (const [user, grant, audience, expectedKey] of [
      ...
      const result = await database.query(`
        ...
        from public.list_authorized_moodle_quiz_attempt_freshness(
          '${user}'::uuid, '${grant}'::uuid,
          'ba000000-0000-4000-8000-000000000001'::uuid,
          'b6000000-0000-4000-8000-000000000001'::uuid,
          'd1000000-0000-4000-8000-000000000002'::uuid,
    ...
    const purge =
      await database.query(`select * from public.purge_moodle_quiz_attempt_observations(
      '2026-07-17T14:30:00Z'::timestamptz, 1)`);
...
async function assertRollback(database) {
  const table = await database.query(
    "select pg_catalog.to_regclass('public.moodle_quiz_attempt_observations') as name"
  );
  const functions = await database.query(`
    ...
    where (namespace.nspname = 'public' and procedure.proname in (
      ...
      'guard_moodle_quiz_attempt_immutable'
    ))`);
  const evidence = await database.query(`
    select
      ...
      (select pg_catalog.count(*)::integer from public.sync_runs where entity_type = 'quiz_attempts_projection') as phase6h2_runs`);
  if (
    table.rows[0]?.name !== null ||
    functions.rows[0]?.count !== 0 ||
    evidence.rows[0]?.phase6g_observations !== 6 ||
    evidence.rows[0]?.phase6g_runs !== 6 ||
    evidence.rows[0]?.phase6h2_runs !== 6
  ) {
    throw new Error(
...
try {
  ...
  let serviceCalls = 0;
  await run(database, "phase6h2-forward-1", sql.migration);
  await run(database, "phase6h2-seed-1", sql.seed);
  await run(database, "phase6h2-assertions-1", sql.assertions);
  browserDenials += await assertRuntimeDenials(database);
  serviceCalls += await assertServiceSemantics(database);

  await run(database, "phase6h2-rollback", sql.rollback);
  await assertRollback(database);

  await run(database, "phase6h2-forward-2", sql.migration);
  await run(database, "phase6h2-seed-2", sql.seed);
  await run(database, "phase6h2-assertions-2", sql.assertions);
  browserDenials += await assertRuntimeDenials(database);
  serviceCalls += await assertServiceSemantics(database);
