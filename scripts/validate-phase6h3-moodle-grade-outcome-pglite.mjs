...
const sql = {
  ...
  seed: read("supabase/manual/113_phase6h3_moodle_grade_outcome_fake_seed.sql"),
  assertions: read(
    "supabase/manual/213_phase6h3_moodle_grade_outcome_assertions.sql"
  ),
  rollback: read(
    "supabase/manual/913_phase6h3_moodle_grade_outcome_rollback.sql"
  ),
};
...
async function assertRuntimeDenials(database) {
  ...
  for (const role of ["anon", "authenticated", "service_role"]) {
    await expectDenied(
      database,
      role,
      "select * from public.moodle_grade_outcome_observations",
      "Direct Phase 6H3 table read"
    );
    count += 1;
  }
  const calls = [
    `select * from public.resolve_moodle_grade_outcome_context(
      '40000000-0000-4000-8000-000000000001'::uuid,
      '50000000-0000-4000-8000-000000000001'::uuid,
      'b6000000-0000-4000-8000-000000000001'::uuid,
      'd3000000-0000-4000-8000-000000000001'::uuid)`,
    `select * from public.list_authorized_moodle_grade_outcome_freshness(
      '40000000-0000-4000-8000-000000000001'::uuid,
      '50000000-0000-4000-8000-000000000001'::uuid,
    ...
    `select * from public.purge_moodle_grade_outcome_observations(
  ...
  try {
    ...
    for (const [user, grant, role, audience, authorizedCount] of contexts) {
      const result = await database.query(`
        select active_role, projection_audience, authorized_user_ids,
               ...
               user_mapping_status, grade_item_mapping_status
      ...
      if (
        ...
        row?.grade_item_mapping_status !== "exact"
      ) {
        throw new Error(
    ...
    for (const [user, grant, audience, expectedKey] of [
      ...
      const result = await database.query(`
        ...
        from public.list_authorized_moodle_grade_outcome_freshness(
          '${user}'::uuid, '${grant}'::uuid,
          'ba000000-0000-4000-8000-000000000001'::uuid,
          'b6000000-0000-4000-8000-000000000001'::uuid,
          'd3000000-0000-4000-8000-000000000001'::uuid,
    ...
    const purge =
      await database.query(`select * from public.purge_moodle_grade_outcome_observations(
      '2026-07-17T14:30:00Z'::timestamptz, 1)`);
...
async function assertRollback(database) {
  const table = await database.query(
    "select pg_catalog.to_regclass('public.moodle_grade_outcome_observations') as name"
  );
  const functions = await database.query(`
    ...
    where (namespace.nspname = 'public' and procedure.proname in (
      ...
      'guard_moodle_grade_outcome_immutable'
    ))`);
  const evidence = await database.query(`
    select
      ...
      (select pg_catalog.count(*)::integer from public.sync_runs where entity_type = 'grade_outcomes_projection') as phase6h3_runs`);
  if (
    table.rows[0]?.name !== null ||
    functions.rows[0]?.count !== 0 ||
    evidence.rows[0]?.phase6g_observations !== 6 ||
    evidence.rows[0]?.phase6g_runs !== 6 ||
    evidence.rows[0]?.phase6h3_runs !== 6
  ) {
    throw new Error(
...
try {
  ...
  let serviceCalls = 0;
  await run(database, "phase6h3-forward-1", sql.migration);
  await run(database, "phase6h3-seed-1", sql.seed);
  await run(database, "phase6h3-assertions-1", sql.assertions);
  browserDenials += await assertRuntimeDenials(database);
  serviceCalls += await assertServiceSemantics(database);

  await run(database, "phase6h3-rollback", sql.rollback);
  await assertRollback(database);

  await run(database, "phase6h3-forward-2", sql.migration);
  await run(database, "phase6h3-seed-2", sql.seed);
  await run(database, "phase6h3-assertions-2", sql.assertions);
  browserDenials += await assertRuntimeDenials(database);
  serviceCalls += await assertServiceSemantics(database);
